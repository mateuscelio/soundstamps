...
export  const store = {
    ...
    deleteTsByLink: async function (soundLink, timestampLink) {
        ...
        sound.timestamps = sound.timestamps.filter(ts => ts.link !== timestampLink);
    ...
    updateSoundTimestamp: async function (sound)  {
        ...
        if (foundSound) {
            ...
            updatedSound = {
                ...
                link: sound.link,
                timestamps: [
                    { link: sound.timestampLink, time: sound.timestamp },
                ],
            }
        }
    ...
    upInsertSound: async function (updateSound) {
        ...
        const soundIndex = sounds.findIndex((sound) => sound.link == updateSound.link);
        if (soundIndex > -1) {
